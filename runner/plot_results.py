...
def plot_iterations_vs_tolerance(df, output_dir="results", show=False):
    ...
    for matrix in matrices:
        ...
        output_path = Path(output_dir) / f"iterations_vs_tolerance_{matrix.replace('.mtx', '')}.svg"
...
def plot_iterations_vs_tolerance_no_gradient(df, output_dir="results", show=False):
    ...
    for matrix in spa_matrices:
        ...
        output_path = Path(output_dir) / f"iterations_vs_tolerance_{matrix.replace('.mtx', '')}_no_gradient.svg"
        plt.savefig(output_path, format='svg')
        print(f"OK: Salvato: {output_path}")
...
def plot_time_vs_tolerance(df, output_dir="results", show=False):
    ...
    for matrix in matrices:
        ...
        output_path = Path(output_dir) / f"time_vs_tolerance_{matrix.replace('.mtx', '')}.svg"
        plt.savefig(output_path, format='svg')
        print(f"OK: Salvato: {output_path}")
...
def plot_error_vs_tolerance(df, output_dir="results", show=False):
    ...
    for matrix in matrices:
        ...
        output_path = Path(output_dir) / f"error_vs_tolerance_{matrix.replace('.mtx', '')}.svg"
        plt.savefig(output_path, format='svg')
        print(f"OK: Salvato: {output_path}")
...
def plot_method_comparison(df, output_dir="results", show=False):
    ...
    for matrix in matrices:
        ...
        for method in methods:
            ...
            if len(converged) > 0:
                ax.plot(converged['tolerance'], converged['iterations'], 
                       marker='o', label=method, color=METHOD_COLORS.get(method), linewidth=2)
        ax.set_xscale('log')
        ...
        for method in methods:
            ...
            if len(converged) > 0:
                ax.plot(converged['tolerance'], converged['time'], 
                       marker='s', label=method, color=METHOD_COLORS.get(method), linewidth=2)
        ax.set_xscale('log')
        ax.set_xlabel('Tolerance', fontsize=11)
        ...
        for method in methods:
            ...
            if len(converged) > 0:
                ax.plot(converged['tolerance'], converged['relative_error'], 
                       marker='^', label=method, color=METHOD_COLORS.get(method), linewidth=2)
        ax.set_xscale('log')
        ax.set_yscale('log')
        ax.set_xlabel('Tolerance', fontsize=11)
        ...
        output_path = Path(output_dir) / f"method_comparison_{matrix.replace('.mtx', '')}.svg"
...
def main():
    ...
    parser.add_argument(
        "--plots",
        nargs="+",
        choices=["iterations", "iterations-no-gradient", "time", "error", "comparative", "comparison", "all"],
        default=["all"],
    ...
    if "all" in plot_types:
        plot_types = ["iterations", "iterations-no-gradient", "time", "error", "comparative", "comparison"]
